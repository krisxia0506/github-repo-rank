-- Migration: Add closed_issues_count and closed_prs_count to current_rankings view
-- This allows the frontend to display total (open + closed) counts

-- Drop the existing view
DROP VIEW IF EXISTS current_rankings;

-- Recreate the view with closed counts included
...
SELECT
  ...
  DENSE_RANK() OVER (ORDER BY (rs.open_issues_count + rs.closed_issues_count) DESC) as issues_rank,
  DENSE_RANK() OVER (ORDER BY (rs.open_prs_count + rs.closed_prs_count) DESC) as prs_rank,
